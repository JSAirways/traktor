...
class ContentController extends Controller
{
    ...
    protected function getAvailableUsers(User $user): Collection
    {
        if ($user->isAdmin()) {
            ...
            return User::select('id', 'profile_name', 'slug', 'email', 'role', 'parent_id', 'profile_picture', 'profile_picture_category')
        ...
        if ($user->children()->exists()) {
            $users = $users->merge(
                $user->children()
                    ->select('id', 'profile_name', 'slug', 'email', 'role', 'parent_id', 'profile_picture', 'profile_picture_category')
                    ->orderBy('profile_name')
                    ->get()
            );
        }
