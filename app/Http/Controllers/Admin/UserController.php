...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $users = $query->orderBy('profile_name')->paginate(15)->withQueryString();
    ...
    public function approve(User $user, Request $request)
    {
        ...
        return back()->with('success', __('messages.user_approved', ['profile_name' => $user->profile_name]));
    }
    ...
    public function reject(User $user, Request $request)
    {
        ...
        return back()->with('success', __('messages.user_rejected', ['profile_name' => $user->profile_name]));
    }
    ...
    public function approveFromEmail(User $user, Request $request)
    {
        ...
        return redirect()->route('welcome')
            ->with('success', __('messages.user_approved', ['profile_name' => $user->profile_name]));
    }
    ...
    public function rejectFromEmail(User $user, Request $request)
    {
        ...
        return redirect()->route('welcome')
            ->with('success', __('messages.user_rejected', ['profile_name' => $user->profile_name]));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($isChild) {
            ...
            // For children, profile_name is the display name and must be unique per parent
            $rules['profile_name'] = [
                'required',
                'string',
                'max:255',
                Rule::unique('users', 'profile_name')->where(function ($query) use ($request) {
    ...
    public function editProfile()
    {
        ...
        $currentPin = $user->getViewPin();
        $currentAdminPin = $user->getAdminPin();
        
        return view('admin.users.edit', compact('user', 'catGifs', 'isSelfEdit', 'currentPin', 'currentAdminPin'));
    }
    ...
    public function convertToParent(ConvertChildToParentRequest $request, User $user)
    {
        ...
        return redirect()->route('admin.users.edit', $user)
            ->with('success', __('admin.child_converted_to_parent', ['name' => $user->profile_name]));
    }
