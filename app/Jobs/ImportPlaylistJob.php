...
class ImportPlaylistJob implements ShouldQueue
{
    ...
    {
        try {
            // Fetch playlist metadata and videos
            $playlistMetadata = $youtubeService->fetchPlaylistMetadata($this->playlistId);
            $playlistVideos = $youtubeService->fetchPlaylistVideos($this->playlistId);
            
            // Create playlist with channel info
            $contentService->createPlaylistWithChannel(
                $playlistMetadata,
                $playlistVideos,
                $this->userId,
                $this->channelInfo
            ...
            Log::info('Playlist imported successfully', [
                ...
                'user_id' => $this->userId,
                'video_count' => count($playlistVideos),
            ]);
        } catch (\Exception $e) {
            Log::error('Playlist import failed', [
                'playlist_id' => $this->playlistId,
                'user_id' => $this->userId,
                'error' => $e->getMessage(),
    ...
    {
        Log::error('Playlist import job failed permanently', [
            'playlist_id' => $this->playlistId,
            'user_id' => $this->userId,
